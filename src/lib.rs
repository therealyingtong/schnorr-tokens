...
impl From<an23_proxy_signature::Signature<ark_grumpkin::Projective>> for Signature {
    fn from(sig: an23_proxy_signature::Signature<ark_grumpkin::Projective>) -> Self {
        Signature {
...
pub fn sign(params: CurvePoint, sk: Fr, message: Fr, policy: Option<u64>) -> Signature {
    ...
    let policy = policy.map(|p| an23_proxy_signature::Policy { amount: p});
...
impl From<an23_proxy_signature::SigningToken<ark_grumpkin::Projective>> for SigningToken {
    fn from(token: an23_proxy_signature::SigningToken<ark_grumpkin::Projective>) -> Self {
        SigningToken {
            z0: token.z0.into(),
            c0: token.c0.into(),
            m0: token.m0.into(),
